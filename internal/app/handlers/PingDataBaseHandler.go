...
import (
	...
	"HappyKod/ServiceShortLinks/internal/storage/usersstorage/pguserssotorage"
...
func PingDataBaseHandler(c *gin.Context) {
	...
	userStorage, err := pguserssotorage.New(cfg.DataBaseURL)
	if err != nil {
		log.Println(constans.ErrorConnectStorage, err)
		http.Error(c.Writer, constans.ErrorConnectStorage, http.StatusInternalServerError)
		return
	...
	if err = userStorage.Ping(); err != nil {
		log.Println(constans.ErrorConnectStorage, err)
		http.Error(c.Writer, constans.ErrorConnectStorage, http.StatusInternalServerError)
		return
	}
